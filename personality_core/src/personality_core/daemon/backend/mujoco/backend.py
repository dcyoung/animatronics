"""MuJoCo simulation backend for Personality Core.

.. note::
   This is a scaffold — the MuJoCo scene and actuator mapping need to be
   filled in once a MJCF / URDF model is available.  Until then it behaves
   identically to the mockup-sim backend.
"""
...
class MujocoBackend(Backend):
    """MuJoCo physics-based simulation backend (stub)."""
    ...
    def __init__(
        ...
        # TODO: Load MuJoCo model and viewer here once available.
        self.logger.warning(
            "MujocoBackend is a stub — running in mockup-sim mode until "
            "a MuJoCo model is integrated."
        )

    # ------------------------------------------------------------------
    # Control loop  (mockup behaviour until MuJoCo model is wired up)
    # ------------------------------------------------------------------
    ...
    def run(self) -> None:
        ...
        while not self.should_stop.is_set():
            ...
            # TODO: replace with mujoco.mj_step
    ...
    def get_status(self) -> "MujocoBackendStatus":
        ...
        return MujocoBackendStatus(motor_control_mode=self._motor_control_mode)

    def get_present_head_joint_positions(
        self,
    ) -> Annotated[npt.NDArray[np.float64], (3,)]:
        """Return current head joint positions ``(3,)``."""
